...
- ~bootstrap.sh~              :: Generate development scripts, Makefile, and setup vendor links based on this literate configuration.
...
#+begin_src sh :mkdirp yes :tangle ./scripts/start-deployment-environment.sh :tangle-mode (identity #o555)
  cd infra
  guix time-machine -C channels.scm -- shell -m manifest.scm -CN -E '^LANG$' -E '^TERM$' -E '^AWS.*$'
#+end_src
...
#+begin_src sh :mkdirp yes :tangle ./scripts/vendor-deps.sh :tangle-mode (identity #o555)
  [ -e lib/MathJax ] && rm -r lib/MathJax
  cp -LR $(guix time-machine -C channels.scm -- shell -Df guix.scm -- bash -c 'echo $GUIX_ENVIRONMENT')/share/javascript/mathjax lib/MathJax
  chmod -R +w lib/MathJax
#+end_src
